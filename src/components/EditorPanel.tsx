...
// Dieses Feld kennt vier Abschnitte. `EditorSection` hat inzwischen einen
// fuenften ('advanced'), den es hier nicht gibt — darum die eigene Auswahl
// statt des ganzen Typs, sonst verlangt Record<> einen Schalter fuer einen
// Abschnitt, den dieses Feld gar nicht zeichnet.
type Section = Exclude<EditorSection, 'advanced'>;
...
export default function EditorPanel(props: {
  ...
  useEffect(() => {
    if (!focusReq) return;
    // Der Compiler-Regelsatz kam nach diesem Feld. Hier ist der Effekt richtig:
    // er reagiert auf eine Anfrage von aussen (Klick in der Vorschau), nicht auf
    // eigenen Zustand. App reicht `focusReq` derzeit gar nicht durch, der Zweig
    // laeuft also nie — er bleibt fuer den Tag, an dem die Vorschau wieder in
    // die Felder zeigt.
